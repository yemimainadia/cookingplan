$(document).ready(function() {
    ...
    $.ajax(settings).done(function(response) {
        ...
        $('.dropdown-item').on('click', function() {
            ...
            //diubah dulu si tanggal yang diklik dari string ke JSON pake JSON Stringify
            var jsonDate = JSON.stringify({ "date": scheduleDate });
            var settings = {
                    "async": true,
                    "crossDomain": true,
                    "url": "https://cookey-8f6f.restdb.io/rest/recipe?q=" + jsonDate,
                    "method": "GET",
                    "headers": {
                        "content-type": "application/json",
                        "x-apikey": keyApi,
                ...
                // minta tampilin hasilnya kalo datanya ada
            $.ajax(settings).done(function(response) {
                console.log(response);
                // hapus resep di html dl baru dioverride sm resep yg baru
                $('.table-recipe').html('');
                // trus override 
                //memisahkan data dari array (copas line#19 persis)
                $.each(response, function(key, value) {
                ...
                //showing recipe details lagi berdasarkan id resep yang dipilih
                // copas lagi dari line #39
                $('.btn-view').on('click', function() {
                    //pake this biar ga ke button view yg lain tp ke button view yg diplih
                    var recipeID = $(this).attr('data-recipeID');
                    var settings = {
    ...
    // pindah ke halaman signup/registrasi customer
